package glacialExpedition.common.core;
...
public class ControllerImpl implements Controller {
    ...
    public String exploreState(String stateName) {
        ...
        List<Explorer> suitableExplores = this.explorerRepository.getCollection().stream()
                .filter(explorer -> explorer.getEnergy() > 50)
                .collect(Collectors.toList());

        if(suitableExplores.isEmpty()){
            throw new IllegalArgumentException(ExceptionMessages.STATE_EXPLORERS_DOES_NOT_EXISTS);
        }
        Mission mission = new MissionImpl();
        mission.explore(state,suitableExplores);

        long retiredExplorers = suitableExplores.stream().filter(explorer -> explorer.getEnergy() == 0).count();
        this.count ++;
    ...
    public String finalResult() {
        ...
        result.append(String.format(ConstantMessages.FINAL_STATE_EXPLORED, this.count))
